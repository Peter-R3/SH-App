...
(async()=>{
    ...
    try {
        ...
        for(const file of ['styles.css','realm-hub.css','realm-planner.css','game-pause.css','achievements.css','game-history.css','diagnostics.css']) await page.addStyleTag({content:fs.readFileSync(path.join(root,file),'utf8')});
        await page.evaluate(()=>{
        ...
        await page.evaluate(()=>{showAuthenticatedApp('Peter');openManagementScreen();});
